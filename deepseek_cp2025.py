...
class LotteryAnalyzer:
    ...
    def analyze_dlt(self):
        ...
        # 获取最近10期数据进行分析
        recent_10_data = self.dlt_df.tail(10)
        ...
        # 最近10期数据分析
        recent_front_numbers = pd.concat([recent_10_data[col] for col in self.dlt_front_cols])
        recent_back_numbers = pd.concat([recent_10_data[col] for col in self.dlt_back_cols])
        ...
        print(f"   前区热门号码(最近10期)：{recent_front_hot}")
        print(f"   前区冷门号码(最近10期)：{recent_front_cold[:5]}...")  # 只显示前5个
        print(f"   后区热门号码(最近10期)：{recent_back_hot}")
        print(f"   后区冷门号码(最近10期)：{recent_back_cold}")
    ...
    def analyze_ssq(self):
        ...
        # 获取最近10期数据进行分析
        recent_10_data = self.ssq_df.tail(30)
        ...
        # 最近10期数据分析
        recent_red_numbers = pd.concat([recent_10_data[col] for col in self.ssq_red_cols])
        recent_blue_numbers = pd.concat([recent_10_data[col] for col in self.ssq_blue_cols])
        ...
        print(f"   红球热门号码(最近10期)：{recent_red_hot}")
        print(f"   红球冷门号码(最近10期)：{recent_red_cold[:5]}...")  # 只显示前5个
        print(f"   蓝球热门号码(最近10期)：{recent_blue_hot}")
        print(f"   蓝球冷门号码(最近10期)：{recent_blue_cold}")
    ...
    def analyze_pl5(self):
        ...
        for i, col in enumerate(self.pl5_cols):
            if col in self.pl5_df.columns:
                ...
                print(f"   {col}热门数字(最近10期)：{recent_hot}")
                print(f"   {col}冷门数字(最近10期)：{recent_cold}")
    ...
    def recommend_dlt_numbers(self):
        ...
        # 获取最近10期数据
        recent_data = self.dlt_df.tail(10)
...
<body>
    <div class="container">
        ...
        <!-- 核心发现 -->
        <div class="section">
            <div class="section-title">🔍 核心发现（基于最近10期数据）</div>
            <div class="lottery-grid">
        ...
        <div class="section">
            ...
            <div class="recommendation" style="margin-top: 15px;">
                ...
                <ul>
                    <li>本推荐基于<b>最近10期数据的热冷平衡策略</b>，更加科学合理</li>
