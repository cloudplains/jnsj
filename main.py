...
import socket
import ssl
import argparse  # 添加 argparse 用于命令行参数解析
...
def clean_urls_file():
    ...
    清理urls.txt文件中的无效链接
    """
    urls_file_path = 'assets/urls.txt'
    
    # 读取原始URL列表
    original_urls = read_txt_to_array(urls_file_path)
    print(f"原始URL数量: {len(original_urls)}")
    
    # 验证URL并收集有效的URL
    valid_urls = []
    for url in original_urls:
        if url.startswith('#'):  # 保留注释行
...
def main():
    # 解析命令行参数
    parser = argparse.ArgumentParser(description='处理直播源')
    parser.add_argument('--clean-urls', action='store_true', help='清理urls.txt中的无效链接')
    args = parser.parse_args()
    
    # 如果指定了清理参数，则只清理urls.txt并退出
    if args.clean_urls:
        clean_urls_file()
        return
    ...
    def is_channel_full(channel_name, target_list):
        ...
        return count >= MAX_SOURCES_PER_CHANNEL
